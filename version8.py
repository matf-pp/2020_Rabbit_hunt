...
class rabbit(object):
    ...
    def number_of_good_bushes(self):
        ...
        for bush in good_bushes:
            if algorithms.manhattan_distance(bush, self.position) + 1 < algorithms.manhattan_distance(bush,
                                                                                                      self.fox_last_seen[
                                                                                                          -1]):
                sol += 1
        return sol
...
def set_table():
    ...
    r = rabbit((random.randrange(10, 16), random.randrange(10, 16)), (255, 255, 255))
    (r_x, r_y) = r.position
    options = [(r_x + i, r_y + 9 - i) for i in range(10)]
    options += [(r_x - i, r_y + 9 - i) for i in range(10)]


    f_pos = random.choice(options)
    while outOffBoundaries(f_pos):
        f_pos = random.choice(options)

    f = fox(f_pos, (255, 51, 0))
    set_bushes(80)
