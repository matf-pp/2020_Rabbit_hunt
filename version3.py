...
class rabbit(object):
        ...
        if self.fox_last_seen[-1] != None:
            ...
            for (dx, dy) in [(0, 1), (1, 0), (-1, 0), (0, -1)]:
                ...
                if curr_position[0] in [0, 1, 24, 23] or curr_position[1] in [0, 1, 24, 23]:
                    curr_cost += 30
                '''
                dist_from_fox = abs(curr_position[0] - self.fox_last_seen[-1][0]) + abs(curr_position[1] - self.fox_last_seen[-1][1])
    ...
    def number_of_good_bushes(self):
        ...
        for bush in good_bushes:
            if algorithms.manhattan_distance(bush, self.position) < algorithms.manhattan_distance(bush, self.fox_last_seen[-1]):
                sol += 1
        return sol 
...
def refreshWindow(surface, bush_coords):
    ...
    surface.blit(textSurface, textRect)
    textSurface, textRect = text_objects ("Step: "+str(number_of_steps//2), (0,0,0),"medium")
...
def main():
    ...
    while game:
        ...
        if bReset.pressed():
            main()
        if bReplay.pressed() and number_of_steps >= 2:
